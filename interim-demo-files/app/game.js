...
class GameFlowController {
  ...
  togglePause() {
    ...
    if (this.gamePaused) {
      ...
      if (this.game1Board) this.game1Board.isAnimating = false; // unfreeze the lobster
...
class LobsterToken {
  ...
  getNextStep(blockedSet, boardSquares, gridWidth, gridHeight) {
    ...
    // If there's no path, OR we're already at a dead end, the lobster is trapped
    if (!path || path.length <= 1) {
      return { nextPos: null, escapedIfMove: false };  // Singals lobster has been caught
    }
...
class Game1Board {
  ...
  clickHexTile(x, y) {
    ...
    // If nextPos is null, the lobster has nowhere to go - Win Cindition for player
    if (!nextPos) {
      this.gameOver = true;
  ...
  // Uses sequential setTimeout for CSS animation timing (turn: 120ms, wiggle: 120ms, jump: 280ms)
  animateTurnWiggleJump(nextPos, escapedIfMove) {
    this.isAnimating = true;
    this.jumpCounter++;

    const stepDelay = GAME_SETTINGS.LOBSTER.moveStepDelay;
    const jumpDelay = GAME_SETTINGS.LOBSTER.jumpDuration;

    // Phase 1: Turn to face movement direction
    ...
    // Phase 2: Wiggle animation (after turn completes)
    setTimeout(() => {
      this.render();
      ...
      // Phase 3: Jump to new position (after wiggle completes)
      setTimeout(() => {
        ...
        // Phase 4: Cleanup and post-movement logic (after jump completes)
        setTimeout(() => {
          const tile4 = this.getCurrentLobsterTile();
          if (tile4) tile4.classList.remove('lobster-jump');
          this.isAnimating = false;

          // Show movement message periodically (every 3-4 jumps)
          ...
          // Handle escape if lobster reached edge
          if (escapedIfMove) {
            if (this.controller.helpSystem) {
  ...
  triggerEscapeAnimation(tile) {
    ...
    // Hide speech bubble during escape
    this.activeBubble = null;
...
class SmartHelpSystem {
  ...
  // Show the help module and pause whatever game is running
  open() {
    this.createModal();
    this.isOpen = true;
    document.body.style.overflow = 'hidden';  // stop background from scrolling

    // Freeze the game so player can read without losing time
    if (this.controller.currentState === 'GAME1') {
      if (!this.controller.gamePaused) {
        this.controller.gamePaused = true;
        if (this.controller.gameTimer) clearInterval(this.controller.gameTimer);
        if (this.controller.game1Board) this.controller.game1Board.isAnimating = true;
      }
    } else if (this.controller.currentState === 'GAME3') {
      if (this.controller.game3Board && !this.controller.game3Board.isPaused) {
        this.controller.game3Board.isPaused = true;
      }
    }
    this.controller.audio.pauseGameSounds(this.controller.currentState);
  }

  // ^ Note not applicable to game 2 as it's turn-based and doesn't have a timer to pause,
  //  so just leaves it running in the background
  ...
  close() {
    ...
    // Unfreeze the game and restart the timer from where it left off from
    if (this.controller.currentState === 'GAME1') {
      if (this.controller.gamePaused) {
        this.controller.gamePaused = false;
        if (this.controller.game1Board) this.controller.game1Board.isAnimating = false;
        if (this.controller.gameTimer) clearInterval(this.controller.gameTimer);
        this.controller.gameTimer = setInterval(() => {
          ...
          if (this.controller.timeRemaining <= 0) {
            ...
            setTimeout(() => this.controller.setGameFlowState('GAME2_READY'), GAME_SETTINGS.TIMING.stateTransitionDelay);
          }
        }, 1000);
      }
    } else if (this.controller.currentState === 'GAME3') {
      if (this.controller.game3Board && this.controller.game3Board.isPaused) {
        this.controller.game3Board.isPaused = false;
      }
    }
    this.controller.audio.resumeForState(this.controller.currentState);
  }
  ...
  // ---------------------------------------------------------------
  // ANALYTICS (not implemented - would have tracked player stats)
  // ---------------------------------------------------------------
  // The idea was to save how many lobsters the player caught/lost
  // and use that to show tailored help tips. Didn't have time to
  // finish this feature so it's all commented out.
  //
  // recordLobsterCaught() {
  //   const count = parseInt(localStorage.getItem(GAME_SETTINGS.STORAGE_KEYS.lobstersCaught) || '0') + 1;
  //   localStorage.setItem(GAME_SETTINGS.STORAGE_KEYS.lobstersCaught, count.toString());
  //   this.playerContext.lobstersCaught = count;
  // }
  //
  // recordLobsterEscaped() {
  //   const count = parseInt(localStorage.getItem(GAME_SETTINGS.STORAGE_KEYS.lobstersEscaped) || '0') + 1;
  //   localStorage.setItem(GAME_SETTINGS.STORAGE_KEYS.lobstersEscaped, count.toString());
  //   this.playerContext.lobstersEscaped = count;
  // }
  //
  // markAsPlayed() {
  //   localStorage.setItem(GAME_SETTINGS.STORAGE_KEYS.playedBefore, 'true');
  //   localStorage.setItem(GAME_SETTINGS.STORAGE_KEYS.lastPlayed, new Date().toISOString());
  //   const gamesPlayed = parseInt(localStorage.getItem(GAME_SETTINGS.STORAGE_KEYS.gamesPlayed) || '0') + 1;
  //   localStorage.setItem(GAME_SETTINGS.STORAGE_KEYS.gamesPlayed, gamesPlayed.toString());
